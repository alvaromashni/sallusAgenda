...
public interface ScheduleRepository extends JpaRepository<Schedule, Long> {
        ...
        @Query("select s.idSchedule as idSchedule, p.personalData.name as patientName, pu.personalData.name as professionalName, p.personalData.email as patientEmail, pu.personalData.email as professionalUserEmail, s.consultationDate as consultationDate, s.consultationTime as consultationTime, s.consultationDescription as consultationDescription from Schedule s join s.patient p join s.professionalUser pu where pu.idProfessionalUser = :idProfessionalUser and s.consultationDate = :date")
        ...
        void softDeleteById(@Param("idSchedule") Long id);
        boolean existsByConsultationTime(LocalTime hour);
}
